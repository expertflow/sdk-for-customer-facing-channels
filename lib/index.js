...
function initiate_call(calledNumber, DN, mediaType, authData, callback, callType) {
    ...
    if (userAgent !== null && userAgent !== undefined) {
        ...
        request_1.extraHeaders.push('X-Customer-Name:' + authData.customerName);
        request_1.extraHeaders.push('X-Agent-Extension:' + authData.agentExtension);
        request_1.extraHeaders.push('X-Customer-Number:' + authData.customerNumber);
        // request.extraHeaders.push('X-Channel:' + authData.channel);
        request_1.extraHeaders.push('X-Customer-Id:' + authData.customerId);
...
function getCalendarId(url, serviceIdentifier, callback) {
    fetch("".concat(url, "/channels/service-identifier/").concat(serviceIdentifier))
        .then(function (response) { return response.json(); })
        .then(function (data) {
        callback(data);
    });
...
function getCalendarEvents(calendarId, url, startTime, endTime, callback) {
    fetch("".concat(url, "/calendars/events?calendarId=").concat(calendarId, "&startTime=").concat(startTime, "&endTime=").concat(endTime))
        .then(function (response) { return response.json(); })
        .then(function (data) {
        callback(data);
    });
